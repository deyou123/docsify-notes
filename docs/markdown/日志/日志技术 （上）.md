...
    9个动态sql标签： trim|where|set|foreach|if|choose|when|otherwise|bind 等。
...
当不使用 @Param 注解时，mybatis 是不认识哪个参数叫什么名字的，尽管在接口中定义了参数的名称，mybatis仍然不认识。这时mybatis将会以接口中参数定义的顺序和SQL语句中的表达式进行映射，这是默认的。
