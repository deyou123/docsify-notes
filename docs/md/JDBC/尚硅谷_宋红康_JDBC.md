...
```sql
# 创建一个用户，可以在任意主机（%）访问数据库，也可指定IP地址
create user ‘jack’@’%’ identified by ’ abc123’;
# 授予增删改查权限给jack
GRANT select,insert,delete,update ON *. * TO ‘jack’@’%’;
# 设置远程登录密码
ALTER USER ‘jack’@’%’ IDENTIFIED WITH mysql_native_password BY ‘abc123’;
